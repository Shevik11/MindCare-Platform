...
const ProfilePage = () => {
  ...
  const onSubmitPsych = async e => {
    ...
    try {
      ...
      setMessage('Psychologist profile updated');
      const res = await axios.get('/api/auth/me');
      setMe(res.data);
    } catch (err) {
      setError(err?.response?.data?.msg || 'Failed to update');
  ...
  const validateFile = file => {
    ...
    if (!allowedTypes.includes(file.type)) {
      toast({
        title: 'Invalid file type. Only PDF and images are allowed',
        status: 'error',
        duration: 5000,
        isClosable: true,
        position: 'top',
  ...
  const handleDocumentUpload = async file => {
    ...
    try {
      ...
      setMe(meRes.data);
      toast({
        title: 'Qualification document uploaded successfully',
        status: 'success',
        duration: 5000,
        isClosable: true,
        position: 'top',
  ...
  const handleDocumentDelete = async () => {
    ...
    try {
      ...
      onDeleteModalClose();
      toast({
        title: 'Qualification document deleted',
        status: 'success',
        duration: 5000,
        isClosable: true,
        position: 'top',
